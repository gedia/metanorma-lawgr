...
* *Navbar toggle:* `scripts_override.html` overrides the base isodoc toggle script to (a) animate the toggle button position in sync with the navbar, and (b) maintain a minimum body `margin-left` (toggle width + gap) when collapsed, so content never overlaps the widget
