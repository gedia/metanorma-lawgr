...
| Paragraph
| Παράγραφος
| Arabic, dot-separated (1, 2… or 5.2.8.1); hidden when article has only one
| 1.
|===
...
The displayed number of a paragraph is derived exclusively from its position within the *article/subarticle/paragraph* ancestry.
Organisational levels (Book, Part, Section, Chapter) exist in the model and have their own numbering, but their numbers are *not* incorporated into paragraph numbering.
...
* *Good-to-have:* PDF (via XSL-FO / `IsoDoc::Lawgr::PdfConvert`)
...
. *Presentation XML* → `IsoDoc::Lawgr::PdfConvert` → *PDF* (good-to-have)

=== Presentation XML customisations

`IsoDoc::Lawgr::PresentationXMLConvert` overrides:

* `ol_depth` — selects Greek numeral type based on nesting depth
* `clause1` — renders structural labels (ΒΙΒΛΙΟ ΠΡΩΤΟ, ΜΕΡΟΣ Α', Άρθρο 1, etc.)
* `paragraph_numbering` — computes the dot-separated display number for each paragraph by walking only the article/subarticle/paragraph ancestry (see <<Paragraph numbering rule>>); organisational ancestors (Book, Part, Section, Chapter) are excluded
* `single_paragraph_suppression` — suppresses paragraph numbering when an article has only one paragraph
...
Custom HTML template and SCSS stylesheet providing:

* Uppercase rendering for top-level structural labels (ΒΙΒΛΙΟ, ΜΕΡΟΣ, ΤΜΗΜΑ, ΚΕΦΑΛΑΙΟ)
* Proper indentation for the article/paragraph hierarchy
* Cover page template with ΦΕΚ publication metadata
* Table of contents following the structural hierarchy
...
The `IsoDoc::Lawgr::Xref` class overrides `anchor_struct` to produce Greek law reference labels using the i18n labels.
...
│           ├── pdf_convert.rb           # IsoDoc::Lawgr::PdfConvert (stub)
...
* Section type dispatch for the full Greek law hierarchy (Book through Paragraph, with recursive nesting)
* Dot-separated paragraph numbering derived from article/subarticle/paragraph ancestry
* Greek numeral algorithm (`GreekNumerals` module)
* Custom ordered-list rendering for περίπτωση / υποπερίπτωση / στοιχείο
* Single-paragraph suppression
* i18n labels (Greek)
* Basic HTML output with structural hierarchy rendering
* Basic metadata extraction (law number, ΦΕΚ, date, type, signers)
* Internal cross-references
* Manual bibliography for law-to-law references
...
* Cover page / ΦΕΚ header template polish
* Word (`.doc`) output
